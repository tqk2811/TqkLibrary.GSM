...
namespace TqkLibrary.GSM.PDU
{
    ...
    public class Pdu
    {
        ...
        /// 
        /// </summary>
        /// <param name="desNumber">
        /// +xxxx ( International )<br></br>
        /// 0xxx ( National )
        /// </param>
            ...
            )
        {
            ...
            if (!isNumber)
                throw new InvalidDataException($"'{desNumber}' requied number and/or '+' only");
            ...
            for (int i = 0; i < msgs.Count; i++)
            {
                ...
                pdu.Address = new DestinationAddress()
                {
                    ...
                    //
                    //In contrast, for numbers written without `+' sign the address gets TON=0 (unknown), NPI=1.
                    //In this case the number must adhere to the mobile operator's dial plan,
                    //which means that international numbers must have the international prefix
                    //(00 in most countries, but 011 in the USA) before the country code and numbers for long-distance calls must start with the trunk prefix
                    //(0 in most countries, 1 in the USA) followed by a trunk code.
                    NPI = NumberingPlanIdentification.ISDNTelephoneNumberingPlan,
                    TON = desNumber.StartsWith("+") ? TypeOfNumber.InternationalNumber : TypeOfNumber.Unknown,
                    Address = desNumber.Trim('+').ToDecimalSemiOctets(),
                };
                ...
                //0051000A01 3018111111 00 08 17 46 050003970300 
                //0051000A01 3018111111 00 08 17 46 050003100301 
                //0051000A01 3018111111 00 08 17 1B 050003100302 0020002C0020006300F2006E0020007200751ED3006900200063006800610020006300681EAF00630020006D1ED900740020007600E3
